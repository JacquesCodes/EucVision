...
# 3. CHM Generation from Normalised Clouds (Capped at 7m)
tic("CHM Rasterization complete")
print("Loading normalised catalog and applying strict 7m filter...")

ctg_normalised <- readLAScatalog(normalised_dir)
opt_independent_files(ctg_normalised) <- TRUE
opt_select(ctg_normalised) <- "xyz"

# THE FIX: Drop points above 7m *before* rasterization to prevent TIN interpolation of air noise
opt_filter(ctg_normalised) <- "-drop_z_below 0 -drop_z_above 7"
...
# 5. Metric Extraction & Consolidation
tic("Metric extraction complete")
print("Extracting metrics and building master CHM...")

# Gather the newly generated individual CHMs
chm_files <- list.files(chm_dir, pattern = "\\.tif$", full.names = TRUE)
# Exclude the old Master CHM if it still exists in the folder so it doesn't try to stitch itself
chm_files <- chm_files[!grepl("Master_Site_CHM_Single", chm_files)]

site_chm_vrt <- terra::vrt(chm_files)

# Extract max height directly from the cleanly interpolated VRT
trees$Tree_Height <- exact_extract(site_chm_vrt, trees, 'max')

# Write the final physical TIFF file (enforcing the 7m ceiling natively)
site_chm_clamped <- terra::clamp(site_chm_vrt, lower = 0, upper = 7)
